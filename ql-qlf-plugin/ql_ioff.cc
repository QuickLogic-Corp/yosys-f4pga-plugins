...
static bool is_io_ff(RTLIL::IdString type) { return type.in(ID(io_sdffr), ID(io_sdffnr)); }
...
static bool port_is_const_ones(RTLIL::Cell *cell, RTLIL::IdString port, SigMap &sigmap, const dict<SigBit, State> &const_drivers)
{
...
struct QlIoffPass : public Pass {
    ...
    void execute(std::vector<std::string> args, RTLIL::Design *design) override
    {
        ...
        log("ql_ioff summary:\n");
        log("  promoted: dff=%d dffn=%d io_sdffr=%d io_sdffnr=%d   (total %d)\n", promoted[ID(dff)], promoted[ID(dffn)], promoted[ID(io_sdffr)],
            promoted[ID(io_sdffnr)], total_promoted);
        log("  declined: E used=%d, async reset=%d, D has other consumers=%d\n", declined_enable, declined_async, declined_dfanout);
    }
} QlIoffPass;
