...
struct QlSdpBramTypesPass : public Pass {
	
	QlSdpBramTypesPass() : Pass("ql_sdp_bram_types", "Change SDP36K type to subtypes") {}

	void help() override
	{
		//   |---v---|---v---|---v---|---v---|---v---|---v---|---v---|---v---|---v---|---v---|
		log("\n");
		log("    ql_sdp_bram_types [selection]\n");
		log("\n");
		log("    This pass changes the type of SDP36K cells to different types based on the\n");
	...
	{
		log_header(design, "Executing QL_SDP_BRAM_TYPES pass.\n");
		...
		for (RTLIL::Module* module : design->selected_modules())
			...
			{
				if (cell->type != ID(SDP36K) || !cell->hasParam(ID(MODE_BITS)))
					continue;
				...
				if (FMODE1_i != FMODE2_i) {
					log_debug("Can't change type of mixed use SDP36K block: FMODE1_i = %s, FMODE2_i = %s\n", FMODE1_i ? "true" : "false", FMODE2_i ? "true" : "false");
					continue;
				}
				...
				if (SYNC_FIFO1_i != SYNC_FIFO2_i) {
					log_debug("Can't change type of mixed use SDP36K block: SYNC_FIFO1_i = %s, SYNC_FIFO2_i = %s\n", SYNC_FIFO1_i ? "true" : "false", SYNC_FIFO2_i ? "true" : "false");
					continue;
				}
				...
				if (RMODE_A1_i != WMODE_A1_i) {
					log_warning("Can't change type of misconfigured SDP36K block: Port A1 configured with read width = %d different from write width = %d\n", width_for_mode(RMODE_A1_i), width_for_mode(WMODE_A1_i));
					continue;
				}
				if (RMODE_B1_i != WMODE_B1_i) {
					log_warning("Can't change type of misconfigured SDP36K block: Port B1 configured with read width = %d different from write width = %d\n", width_for_mode(RMODE_B1_i), width_for_mode(WMODE_B1_i));
					continue;
				}
				if (RMODE_A2_i != WMODE_A2_i) {
					log_warning("Can't change type of misconfigured SDP36K block: Port A2 configured with read width = %d different from write width = %d\n", width_for_mode(RMODE_A2_i), width_for_mode(WMODE_A2_i));
					continue;
				}
				if (RMODE_B2_i != WMODE_B2_i) {
					log_warning("Can't change type of misconfigured SDP36K block: Port B2 configured with read width = %d different from write width = %d\n", width_for_mode(RMODE_B2_i), width_for_mode(WMODE_B2_i));
					continue;
				}

				// TODO: For nonsplit blocks, should RMODE_A1_i == RMODE_A2_i etc be checked/enforced?

				std::string type = "SDP36K";
				if (is_fifo) {
					type += "_FIFO_";
					if (sync_fifo)
						type += "SYNC_";
