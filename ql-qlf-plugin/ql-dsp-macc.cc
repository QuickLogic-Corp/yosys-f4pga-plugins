...
static void create_ql_macc_dsp(ql_dsp_macc_pm &pm)
{
    ...
    // DSPv2 type classification (ql_dspv2_types) requires feedback to be
    // constant. The "clear to product" pattern (if clr z<=a*b else z<=z+a*b)
    // needs dynamic feedback which DSPv2 cannot support. Reject and let the
    // mux + accumulate stay as generic cells.
    if (dspv2 && st.mux != nullptr) {
        return;
    }
    ...
    if (st.ff->hasPort(ID(ARST))) {
        ...
        // Sync-reset FF ($sdff / $sdffe). The DSP cell's reset_i is async,
        // so sync resets must be routed to acc_reset_i instead (dspv2) or
        // reset_i (v1, which treats it as sync). Reject if SRST_VALUE is
        // non-zero (cannot reproduce that reset value in hardware).
        if (st.ff->hasParam(ID(SRST_VALUE)) &&
            !st.ff->getParam(ID(SRST_VALUE)).is_fully_zero()) {
            return;
        }
        sync_reset = true;
        if (st.ff->getParam(ID(SRST_POLARITY)).as_int() != 1) {
            rst = pm.module->Not(NEW_ID, st.ff->getPort(ID(SRST)));
        } else {
            rst = st.ff->getPort(ID(SRST));
        }
    } else {
    ...
    if (dspv2) {
        ...
        // Sync resets go to acc_reset_i (synchronous accumulator clear);
        // async resets go to reset_i (already set above). When sync_reset
        // is true, override reset_i back to S0 so the DSP's async reset
        // path is not triggered.
        if (sync_reset) {
            cell->setPort(RTLIL::escape_id("reset_i"), RTLIL::SigSpec(RTLIL::S0));
            cell->setPort(RTLIL::escape_id("acc_reset_i"), rst);
        } else {
        ...
        // output_select_i encoding for ql_dspv2_types control-word:
        // get_control_word() masks output_select to 2 bits ((os & 0b11) << 3).
        // MULTACC requires (os & 0b11) == 1 -> control_word 0b01000 = 8.
        // For MAC patterns the accumulator FF is absorbed into the DSP, so the
        // output is always "combinational" from ql_dspv2_types perspective
        // (no extra dffre on z). Use 1 (0b001) unconditionally.
        cell->setPort(RTLIL::escape_id("output_select_i"), RTLIL::Const(1, 3));
