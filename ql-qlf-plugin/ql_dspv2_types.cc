...
struct QlDSPV2TypesPass : public Pass {
    ...
    static int get_const_port_value(RTLIL::Cell *cell, RTLIL::IdString port_name, SigMap &sigmap, const dict<SigBit, State> &const_drivers)
    {
    ...
    uint32_t get_control_word(uint32_t feedback,      // 3 bits
                              uint32_t output_select, // 2 bits
                              bool zcin_sel, bool padd_sel, bool sub)
    {
    ...
    void add_bitwise_dffre_before_cell_input(RTLIL::Module *module, RTLIL::Cell *cell, RTLIL::IdString port, IdString cell_type, SigMap &sigmap,
                                             const dict<SigBit, State> &const_drivers)
    {
        SigSpec clk, rst;
        if (cell->hasPort(IdString("\\clk")))
            clk = cell->getPort(IdString("\\clk"));
        if (cell->hasPort(IdString("\\reset")))
            rst = cell->getPort(IdString("\\reset"));
        ...
        if (reg_count > 0)
            reg_wire = module->addWire(module->uniquify(stringf("\\%s_%s_reg", log_id(cell), log_id(port))), reg_count);
        ...
        for (int i = 0; i < width; i++) {
            ...
            Cell *dff = module->addCell(module->uniquify(cell_type), cell_type);
    ...
    void add_bitwise_dffre_after_cell_output(RTLIL::Module *module, RTLIL::Cell *cell, RTLIL::IdString port,
                                             IdString cell_type) // output port
    {
        ...
        Wire *reg_wire = module->addWire(module->uniquify(stringf("\\%s_%s_reg", log_id(cell), log_id(port))), width);
    ...
    void replace_drop_net_with_keep_net(RTLIL::Module *module, RTLIL::Cell *cell, RTLIL::IdString keep_port, RTLIL::IdString drop_port)
    {
        // Safety checks
    ...
    void transform_cell_with_ports(RTLIL::Cell *cell, RTLIL::IdString new_type, const pool<RTLIL::IdString> &allowed_ports)
    {
        // 1. Change cell type
    ...
    void make_gnd_bits_unconn(RTLIL::Cell *cell, const pool<RTLIL::IdString> &ports, SigMap &sigmap, const dict<SigBit, State> &const_drivers)
    {
    ...
    // ============================================================================
    static bool is_dffre(const RTLIL::Cell *cell) { return cell->type == IdString("\\dffre") || cell->type == IdString("\\DFFRE"); }

    // ============================================================================
    // Collect all DSP input usages, grouped by canonical signal bit.
    // Pass an empty port filter to collect all input ports.
    // ============================================================================
    static dict<RTLIL::SigBit, std::vector<DSPUsage>> collect_dsp_usages(RTLIL::Module *module, const pool<RTLIL::IdString> &port_filter = {})
    {
        SigMap sigmap(module);
    ...
    // ============================================================================
    static bool get_dffre_q_driver(RTLIL::Module *module, const RTLIL::SigBit &bit, RTLIL::Cell *&ff_cell, int &q_idx)
    {
        if (bit.wire == nullptr) {
    ...
    // ============================================================================
    static RTLIL::SigBit duplicate_ff_bit(RTLIL::Module *module, RTLIL::Cell *orig_ff, int q_idx)
    {
        ...
        RTLIL::Wire *new_q = module->addWire(module->uniquify(stringf("\\%s_dup_q_%d", log_id(orig_ff), q_idx)), 1);

        log_debug("  duplicate_ff_bit: orig_ff=%s q_idx=%d new_wire=%s\n", log_id(orig_ff), q_idx, log_id(new_q));

        RTLIL::Cell *dup_ff = module->addCell(module->uniquify(IdString("\\dffre")), IdString("\\dffre"));

        dup_ff->setPort("\\D", orig_ff->getPort("\\D"));
        dup_ff->setPort("\\C", orig_ff->getPort("\\C"));
        dup_ff->setPort("\\R", orig_ff->getPort("\\R"));
        dup_ff->setPort("\\E", orig_ff->hasPort("\\E") ? orig_ff->getPort("\\E") : SigSpec());
    ...
    // ============================================================================
    static void insert_dummy_buffer(RTLIL::Module *module, const RTLIL::SigBit &src_bit, const DSPUsage &usage)
    {
        static int buf_count = 0;
        std::string idx = stringf("%d", buf_count++);

        log_debug("  insert_dummy_buffer: src_bit=%s DSP=%s port=%s bit[%d] buf_idx=%s\n", log_signal(src_bit), log_id(usage.cell),
                  log_id(usage.port), usage.bit_idx, idx.c_str());

        RTLIL::Wire *buf_wire = module->addWire(module->uniquify("\\dummy_buf_wire_" + idx));
        RTLIL::Cell *buf = module->addCell(module->uniquify("\\dummy_buf_" + idx), ID(buffer));
    ...
    // ============================================================================
    static void duplicate_shared_dffres(RTLIL::Module *module, RTLIL::Cell *target_cell, const pool<RTLIL::IdString> &input_ports)
    {
        SigMap sigmap(module);
    ...
    static void duplicate_dffre_per_dsp_port(RTLIL::Module *module)
    {
        ...
        const pool<RTLIL::IdString> dsp_ports = {IdString("\\a"), IdString("\\b"), IdString("\\c")};
        ...
        for (auto cell : module->cells()) {
            ...
            for (auto &conn : cell->connections()) {
                ...
                if (is_dsp && !dsp_ports.count(port))
                    continue;

                SigSpec sig = sigmap(conn.second);
                for (int i = 0; i < GetSize(sig); i++) {
                    SigBit bit = sigmap(sig[i]);
                    if (!dffre_q_map.count(bit))
                        continue;
        ...
        for (auto &it : sink_map) {
            ...
            for (auto &dg : dsp_groups) {
                ...
                for (auto &u : usages) {
                    ...
                    log_debug("      rewire DSP=%s port=%s bit[%d] -> %s\n", log_id(u.cell), log_id(u.port), u.bit_idx, log_signal(new_q));
                }
    ...
    void execute(std::vector<std::string> args, RTLIL::Design *design) override
    {
        ...
        for (RTLIL::Module *module : design->selected_modules()) {
            ...
            for (RTLIL::Cell *cell : module->selected_cells()) {
                ...
                replace_drop_net_with_keep_net(module, cell, RTLIL::IdString("\\z"), RTLIL::IdString("\\z_cout"));

                if (A_REG) {
                    add_bitwise_dffre_before_cell_input( // QL_DSPV2_A_DFFR
                      module, cell, RTLIL::IdString("\\a"), RTLIL::IdString("\\QL_DSPV2_A_DFFR"), sigmap, const_drivers);
                    // Setting A_REG bit to 0
                    set_param_bit(cell, ID(MODE_BITS), 61, RTLIL::State::S0);
                } else if (A1_REG && A2_REG) { // QL_DSPV2_A_DFFR
                    add_bitwise_dffre_before_cell_input(module, cell, RTLIL::IdString("\\a"), RTLIL::IdString("\\QL_DSPV2_A_DFFR"), sigmap,
                                                        const_drivers);
                    add_bitwise_dffre_before_cell_input( // QL_DSPV2_A2_DFFR
                      module, cell, RTLIL::IdString("\\a"), RTLIL::IdString("\\QL_DSPV2_A2_DFFR"), sigmap, const_drivers);
                    ...
                    add_bitwise_dffre_before_cell_input(module, cell, RTLIL::IdString("\\a"), RTLIL::IdString("\\QL_DSPV2_A2_DFFR"), sigmap,
                                                        const_drivers);
                ...
                if (B_REG) {
                    add_bitwise_dffre_before_cell_input( // QL_DSPV2_B_DFFR
                      module, cell, RTLIL::IdString("\\b"), RTLIL::IdString("\\QL_DSPV2_B_DFFR"), sigmap, const_drivers);
                    ...
                    add_bitwise_dffre_before_cell_input( // QL_DSPV2_B2_DFFR
                      module, cell, RTLIL::IdString("\\b"), RTLIL::IdString("\\QL_DSPV2_B2_DFFR"), sigmap, const_drivers);
                ...
                if (C_REG) {
                    add_bitwise_dffre_before_cell_input( // QL_DSPV2_C_DFFR
                      module, cell, RTLIL::IdString("\\c"), RTLIL::IdString("\\QL_DSPV2_C_DFFR"), sigmap, const_drivers);
                ...
                if (M_REG && OUTPUT_SELECT >= 4) {
                    add_bitwise_dffre_after_cell_output( // QL_DSPV2_Z_DFFR
                      module, cell, RTLIL::IdString("\\z"), RTLIL::IdString("\\dffre"));
                    add_bitwise_dffre_after_cell_output( // dffre
                      module, cell, RTLIL::IdString("\\z"), RTLIL::IdString("\\QL_DSPV2_Z_DFFR"));
                ...
                uint32_t control_word = get_control_word(FEEDBACK, OUTPUT_SELECT, ZCIN_SEL, PRE_ADD, SUBTRACT);

                log_debug("Control Word: %d\n", control_word);
                switch (control_word) {
                case 0b00000000: // MULT
                    transform_cell_with_ports(cell, RTLIL::escape_id("QL_DSPV2_MULT"),
                                              pool<RTLIL::IdString>{ID(a), ID(b), ID(z), ID(feedback), ID(output_select)});
                    break;

                case 0b01011000: // CONCAT_CASCADE
                case 0b01010000: // CONCAT_CASCADE
                    transform_cell_with_ports(cell, RTLIL::escape_id("QL_DSPV2_CONCAT_CASCADE"),
                                              pool<RTLIL::IdString>{ID(a), ID(b), ID(feedback), ID(output_select), ID(z)});
                    break;

                case 0b00001000: // MULTACC
                    transform_cell_with_ports(
                      cell, RTLIL::escape_id("QL_DSPV2_MULTACC"),
                      pool<RTLIL::IdString>{ID(a), ID(b), ID(z), ID(clk), ID(reset), ID(acc_reset), ID(load_acc), ID(feedback), ID(output_select)});
                    break;

                case 0b00001001: // MULTACC_NEG
                    transform_cell_with_ports(
                      cell, RTLIL::escape_id("QL_DSPV2_MULTACC_NEG"),
                      pool<RTLIL::IdString>{ID(a), ID(b), ID(z), ID(clk), ID(reset), ID(acc_reset), ID(load_acc), ID(feedback), ID(output_select)});
                    break;

                case 0b00000010: // PREADDER_MULT
                    transform_cell_with_ports(cell, RTLIL::escape_id("QL_DSPV2_PREADDER_MULT"),
                                              pool<RTLIL::IdString>{ID(a), ID(b), ID(c), ID(feedback), ID(output_select), ID(z)});
                    break;

                case 0b01111110: // PREADDER_MULTADD
                case 0b01110110: // PREADDER_MULTADD
                    make_gnd_bits_unconn(cell,
                                         pool<RTLIL::IdString>{
                                           ID(z_cin),
                                         },
                                         sigmap, const_drivers);
                    transform_cell_with_ports(cell, RTLIL::escape_id("QL_DSPV2_PREADDER_MULTADD"),
                                              pool<RTLIL::IdString>{ID(a), ID(b), ID(c), ID(feedback), ID(output_select), ID(z), ID(z_cin)});
                    break;
                ...
                case 0b01110100: // MULTADD
                    make_gnd_bits_unconn(cell,
                                         ...
                                         sigmap, const_drivers);
                    transform_cell_with_ports(cell, RTLIL::escape_id("QL_DSPV2_MULTADD"),
                                              pool<RTLIL::IdString>{ID(a), ID(b), ID(z), ID(z_cin), ID(feedback), ID(output_select)});
                    break;
                ...
                case 0b01110101: // MULTADD_NEG
                    make_gnd_bits_unconn(cell,
                                         ...
                                         sigmap, const_drivers);
                    transform_cell_with_ports(cell, RTLIL::escape_id("QL_DSPV2_MULTADD_NEG"),
                                              pool<RTLIL::IdString>{ID(a), ID(b), ID(z), ID(z_cin), ID(feedback), ID(output_select)});
                    break;
                ...
                default:
                    ...
                    make_gnd_bits_unconn(cell,
                                         ...
                                         sigmap, const_drivers);
                    transform_cell_with_ports(cell, RTLIL::escape_id("QL_DSPV2"),
                                              pool<RTLIL::IdString>{ID(a), ID(b), ID(c), ID(clk), ID(feedback), ID(load_acc), ID(output_select),
                                                                    ID(reset), ID(acc_reset), ID(z_cin), ID(z)});
                    break;
                }
            }
