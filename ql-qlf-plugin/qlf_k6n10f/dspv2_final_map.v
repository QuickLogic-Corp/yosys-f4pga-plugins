...
// Final-map techmap source for the DSPv2 inference flow.
//
// Lowers `dspv2_*_cfg_ports` wrappers into the hard `QL_DSPV2` primitive,
// packing all per-field cfg-params into the 80-bit `MODE_BITS` parameter the
// hard cell expects. The `(* is_inferred *)` marker is carried over so
// downstream passes (e.g. `ql_dspv2_types`) can distinguish inferred cells
// from user-instantiated ones.
//
// MODE_BITS layout (80 bits, must match QL_DSPV2.v / dspv2_sim.v):
//   {FRAC_MODE, 5'h0, BCOUT_SEL, ACOUT_SEL,
...
// The 16x9 wrapper pads COEFF_0 from 16 to 32 bits with 16'h0.
