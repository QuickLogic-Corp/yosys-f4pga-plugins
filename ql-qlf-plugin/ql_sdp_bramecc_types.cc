        ...
        for (RTLIL::Module *module : design->selected_modules())
            for (RTLIL::Cell *cell : module->selected_cells()) {
                ...
                if (RMODE_A1_i != WMODE_A1_i) {
                    log_warning("Can't change type of misconfigured SDP_ECC36K block: Port A1 configured with read width = %d different from write "
                                "width = %d\n",
                                width_for_mode(RMODE_A1_i), width_for_mode(WMODE_A1_i));
                    continue;
                }
                if (RMODE_B1_i != WMODE_B1_i) {
                    log_warning("Can't change type of misconfigured SDP_ECC36K block: Port B1 configured with read width = %d different from write "
                                "width = %d\n",
                                width_for_mode(RMODE_B1_i), width_for_mode(WMODE_B1_i));
                    continue;
                }
                if (RMODE_A2_i != WMODE_A2_i) {
                    log_warning("Can't change type of misconfigured SDP_ECC36K block: Port A2 configured with read width = %d different from write "
                                "width = %d\n",
                                width_for_mode(RMODE_A2_i), width_for_mode(WMODE_A2_i));
                    continue;
                }
                if (RMODE_B2_i != WMODE_B2_i) {
                    log_warning("Can't change type of misconfigured SDP_ECC36K block: Port B2 configured with read width = %d different from write "
                                "width = %d\n",
